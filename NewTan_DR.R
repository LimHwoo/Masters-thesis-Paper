...
################################ Data load
rm(list=ls())
setwd("/Users/hyunwoo/Desktop/eye_data")
getwd()
...
################################ DR
data = read.csv("eye_data.csv",header=T)
...
for(x in iter){
  ...
  #############################Over-sampling
  #traindata = rbind(traindata,traindata[sample(which(traindata$DR==1),sum(traindata$DR==0)-sum(traindata$DR!=0),replace=T),])
  #traindata = traindata[sample(1:NROW(traindata),50000),]
  #############################Under-sampling
  #traindata = rbind(traindata[sample(which(traindata$DR==0),sum(traindata$DR==1)),],
  #                  traindata[which(traindata$DR==1),])
  ############### SMOTE
  ...
  result_Tan = cl_algorithm(class=class,data=traindata,root_='information',del='class')
  results_Tan=learn_parameter(result_Tan$data,result_Tan,class=class,alpha=1) 
  fit_Tan = bnc('tan_cl','DR',traindata[,c(-9)],smooth=1)
  ...
  fit_Tan2 = bnc('tan_cl','DR',traindata[,c(-1,-2,-9)],smooth=1)
  fit_Tan2$.params=results_Tan2
  end_Tantime2 = Sys.time()
  
  start_Tantime3 = Sys.time()
  result_Tan3 = cl_algorithm(class='class',data=traindata,root_='information',del=c('SEX','AGE_G','DR'))
  results_Tan3=learn_parameter(result_Tan3$data,result_Tan3,class='class',alpha=1) 
  fit_Tan3 = bnc('tan_cl','class',traindata[,c(-1,-2,-8)],smooth=1)
  ...
  fit_Tan3_1 = bnc('tan_cl','class',traindata[,c(-1,-2,-8)],smooth=1)
  ...
  Tan_acc = accuracy(pred_Tan,testdata$DR)
  Tan_tab=table(pred_Tan,testdata$DR,dnn=c("pred","real"))
  Tan_sens = Sensitivity(testdata$DR,pred_Tan)
  Tan_spec = Specificity(testdata$DR,pred_Tan)
  Tan_precision = Precision(testdata$DR,pred_Tan)
  Tan_f1 = F1_Score(testdata$DR,pred_Tan)
  Tan_roc = roc(testdata$DR,Tan_prob[,2],quiet=T)$auc[1] ################### ROC / AUC
  
  pred_Tan2=predict(fit_Tan2,testdata)
  Tan_prob2 = predict(fit_Tan2,testdata,prob=T)
  Tan_acc2 = accuracy(pred_Tan2,testdata$DR)
  Tan_tab2 = table(pred_Tan2,testdata$DR,dnn=c("pred","real"))
  Tan_sens2 = Sensitivity(testdata$DR,pred_Tan2)
  Tan_spec2 = Specificity(testdata$DR,pred_Tan2)
  Tan_precision2 = Precision(testdata$DR,pred_Tan2)
  Tan_f12 = F1_Score(testdata$DR,pred_Tan2)
  Tan_roc2 = roc(testdata$DR,Tan_prob2[,2],quiet=T)$auc[1] ################### ROC / AUC
  ...
  Tan_acc3 = accuracy(pred_Tan3,testdata$DR)
  Tan_tab3 = table(pred_Tan3,testdata$DR,dnn=c("pred","real"))
  Tan_sens3 = Sensitivity(testdata$DR,pred_Tan3)
  Tan_spec3 = Specificity(testdata$DR,pred_Tan3)
  Tan_precision3 = Precision(testdata$DR,pred_Tan3)
  Tan_f13 = F1_Score(testdata$DR,pred_Tan3)
  Tan_roc3 = roc(testdata$DR,apply(Tan_prob3[,9:16],1,sum),quiet=T)$auc[1] ################### ROC / AUC
  ...
  Tan_acc3_1 = accuracy(pred_Tan3_1,testdata$DR)
  Tan_tab3_1 = table(pred_Tan3_1,testdata$DR,dnn=c("pred","real"))
  Tan_sens3_1 = Sensitivity(testdata$DR,pred_Tan3_1)
  Tan_spec3_1 = Specificity(testdata$DR,pred_Tan3_1)
  Tan_precision3_1 = Precision(testdata$DR,pred_Tan3_1)
  Tan_f13_1 = F1_Score(testdata$DR,pred_Tan3_1)
  Tan_roc3_1 = roc(testdata$DR,apply(Tan_prob3_1[,9:16],1,sum),quiet=T)$auc[1] ################### ROC / AUC
  ...
  for(j in 1:length(k_ne)){
    ...
    for(i in 1:k){
      ...
      trainlabel=train[,'DR']
      validlabel=valid[,'DR']
      valid_fit = knn_hamming(traindata=train[,-9],class='DR',testdata=valid[,-9],k=k_ne[j])
  ...
  test_label = testdata[,'DR']
  
  pred <- knn_hamming(traindata , class="DR",testdata, k=opt_k)
  knn_tab = table(pred$new_class,testdata$DR)
  ...
  fit3 = bnc('nb','DR',traindata,smooth=1)
  fit3_pred=predict(fit3,testdata)
  Naive_prob = predict(fit3,testdata,prob=T)
  Naive_tab = table(fit3_pred,testdata$DR)
  
  Naive_sens = Sensitivity(testdata$DR,fit3_pred)
  Naive_spec = Specificity(testdata$DR,fit3_pred)
  Naive_precision = Precision(testdata$DR,fit3_pred)
  Naive_f1 = F1_Score(testdata$DR,fit3_pred)
  
  Naive_acc = accuracy(fit3_pred,testdata$DR)  #####################3 Acc of Naive
  Naive_roc = roc(testdata$DR,Naive_prob[,2],quiet=T)$auc[1] ################# AUC of Naive
  ...
  tree_fit = rpart(DR~.,data=traindata , method='class')
  ...
  tree_acc=accuracy(tree_fit_pred,testdata$DR)
  tree_fit_prob = predict(tree_fit,testdata)
  tree_tab = table(tree_fit_pred,testdata$DR)
  
  tree_sens = Sensitivity(testdata$DR,tree_fit_pred)
  tree_spec = Specificity(testdata$DR,tree_fit_pred)
  tree_precision = Precision(testdata$DR,tree_fit_pred)
  tree_f1 = F1_Score(testdata$DR,tree_fit_pred)
  tree_roc = roc(testdata$DR,tree_fit_prob[,2],quiet=T)$auc[1] ################# AUC of Tree
  ...
  for(j in 1:length(trees)){
    ...
    for(i in 1:k){
      train = traindata[which(traindata$id!=k),]
      valid = traindata[which(traindata$id==k),]
      valid_fit = randomForest(DR~.,data=subset(train, select=-id),mtry=sqrt(length(traindata)-1),ntree=trees[j])
      valid_pred = predict(valid_fit,newdata = subset(valid,select=-id),type='class')
      valid_acc = accuracy(valid_pred,valid[,'DR'])
      valid_result = c(valid_result,valid_acc)
    }
  ...
  traindata = subset(traindata,select=-id)
  ################################# Fitting RF
  RF_fit=randomForest(DR~.,data=traindata , mtry=sqrt(length(data)-1), ntree=opt)
  RF_pred = predict(RF_fit, newdata = testdata , type='class')
  RF_prob = predict(RF_fit, newdata = testdata , type ='prob')
  
  RF_tab = table(RF_pred , testdata$DR)
  RF_acc = accuracy(RF_pred,testdata$DR)
  RF_sens = Sensitivity(testdata$DR,RF_pred)
  RF_spec = Specificity(testdata$DR,RF_pred)
  RF_precision = Precision(testdata$DR,RF_pred)
  RF_f1 = F1_Score(testdata$DR,RF_pred)
  
  RF_roc = roc(testdata$DR,RF_prob[,2],quiet=T)$auc[1]################# AUC of RF
  end_rftime = Sys.time()
  ...
  #write.csv(final_result,paste("eye_data_DR_cv_seed=",seed,".csv"))
  #capture.output(results,file=paste("eye_data_DR_seed=",seed,".txt"))
  n=n+1
}
###################### 
...
write.csv(final_cv_result,paste("DR_Total.csv"))
