...
################################ Data load
rm(list=ls())
setwd("/Users/hyunwoo/Desktop/blood_data")
getwd()
...
################################ STK
data = read.csv("blood_data.csv",header=T)
#data = data[sample(1:NROW(data),100000),]
data$HGB = ifelse(data$SEX==1,ifelse(data$HGB<12,0,ifelse(data$HGB<13,1,ifelse(data$HGB<=16.5,2,3))),data$HGB)
data$HGB = ifelse(data$SEX==2,ifelse(data$HGB<10,0,ifelse(data$HGB<12,1,ifelse(data$HGB<=15.5,2,3))),data$HGB)
data$TCHOL = ifelse(data$TCHOL<200,0,ifelse(data$TCHOL<240,1,2))
data$TG = ifelse(data$TG<150,0,ifelse(data$TG<200,1,ifelse(data$TG<500,2,3)))
data$HDL = ifelse(data$HDL<40,0,ifelse(data$HDL<60,1,2))
data$AGE_G = ifelse(data$AGE_G<5,0,ifelse(data$AGE_G<15,1,ifelse(data$AGE_G<22,2,3)))
...
class='STK'
n=1
iter = sample(1:10000,1,replace=F)
...
for(x in iter){
  ...
  #############################Over-sampling
  # data_over = ubOver(X=traindata[,-9],Y=traindata$STK,k=0)
  # data_over = cbind(data_over$X,data_over$Y)
  # colnames(data_over)[9] = "STK"
  # traindata = data_over
  # #traindata = rbind(traindata,traindata[sample(which(traindata$STK==1),sum(traindata$STK==0)-sum(traindata$STK!=0),replace=T),])
  #traindata = traindata[sample(1:NROW(traindata),50000),]
  #############################Under-sampling
  #traindata = rbind(traindata[sample(which(traindata$STK==0),sum(traindata$STK==1)),],
  #                  traindata[which(traindata$STK==1),])
  ############### SMOTE
  ...
  result_Tan = cl_algorithm(class=class,data=traindata,root_='information',del=c('class'))
  results_Tan=learn_parameter(result_Tan$data,result_Tan,class=class,alpha=1) 
  fit_Tan = bnc('tan_cl','STK',traindata[,c(-10)],smooth=1)
  fit_Tan$.params=results_Tan
  end_Tantime1 = Sys.time()
  
  start_Tantime2 = Sys.time()
  result_Tan2 = cl_algorithm(class=class,data=traindata,root_='information',del=c('SEX','AGE_G','class'))
  results_Tan2=learn_parameter(result_Tan2$data,result_Tan2,class=class,alpha=1) 
  fit_Tan2 = bnc('tan_cl','STK',traindata[,c(-1,-2,-10)],smooth=1)
  fit_Tan2$.params=results_Tan2
  end_Tantime2 = Sys.time()
  
  start_Tantime3 = Sys.time()
  result_Tan3 = cl_algorithm(class='class',data=traindata,root_='information',del=c('SEX','AGE_G','STK'))
  results_Tan3=learn_parameter(result_Tan3$data,result_Tan3,class='class',alpha=1) 
  fit_Tan3 = bnc('tan_cl','class',traindata[,c(-1,-2,-9)],smooth=1)
  fit_Tan3$.params=results_Tan3
  end_Tantime3 = Sys.time()
  
  start_Tantime4 = Sys.time()
  result_Tan4 = cl_algorithm(class='class',data=traindata,root_=result_Tan2$information[1,1],
                             del=c('SEX','AGE_G','STK'))
  results_Tan4=learn_parameter(result_Tan4$data,result_Tan4,class='class',alpha=1) 
  fit_Tan3_1 = bnc('tan_cl','class',traindata[,c(-1,-2,-9)],smooth=1)
  ...
  Tan_acc = accuracy(pred_Tan,testdata$STK)
  Tan_tab=table(pred_Tan,testdata$STK,dnn=c("pred","real"))
  Tan_sens = Sensitivity(testdata$STK,pred_Tan)
  Tan_spec = Specificity(testdata$STK,pred_Tan)
  Tan_precision = Precision(testdata$STK,pred_Tan)
  Tan_f1 = F1_Score(testdata$STK,pred_Tan)
  Tan_roc = roc(testdata$STK,Tan_prob[,2],quiet=T)$auc[1] ################### ROC / AUC
  
  pred_Tan2=predict(fit_Tan2,testdata)
  Tan_prob2 = predict(fit_Tan2,testdata,prob=T)
  Tan_acc2 = accuracy(pred_Tan2,testdata$STK)
  Tan_tab2 = table(pred_Tan2,testdata$STK,dnn=c("pred","real"))
  Tan_sens2 = Sensitivity(testdata$STK,pred_Tan2)
  Tan_spec2 = Specificity(testdata$STK,pred_Tan2)
  Tan_precision2 = Precision(testdata$STK,pred_Tan2)
  Tan_f12 = F1_Score(testdata$STK,pred_Tan2)
  Tan_roc2 = roc(testdata$STK,Tan_prob2[,2],quiet=T)$auc[1] ################### ROC / AUC
  
  pred_Tan3=predict(fit_Tan3,testdata)
  pred_Tan3 = as.factor(ifelse(as.integer(pred_Tan3)<9,0,1))
  Tan_prob3 = predict(fit_Tan3,testdata,prob=T)
  Tan_acc3 = accuracy(pred_Tan3,testdata$STK)
  Tan_tab3 = table(pred_Tan3,testdata$STK,dnn=c("pred","real"))
  Tan_sens3 = Sensitivity(testdata$STK,pred_Tan3)
  Tan_spec3 = Specificity(testdata$STK,pred_Tan3)
  Tan_precision3 = Precision(testdata$STK,pred_Tan3)
  Tan_f13 = F1_Score(testdata$STK,pred_Tan3)
  Tan_roc3 = roc(testdata$STK,apply(Tan_prob3[,9:16],1,sum),quiet=T)$auc[1] ################### ROC / AUC
  
  pred_Tan3_1=predict(fit_Tan3_1,testdata)
  pred_Tan3_1 = as.factor(ifelse(as.integer(pred_Tan3_1)<9,0,1))
  Tan_prob3_1 = predict(fit_Tan3_1,testdata,prob=T)
  Tan_acc3_1 = accuracy(pred_Tan3_1,testdata$STK)
  Tan_tab3_1 = table(pred_Tan3_1,testdata$STK,dnn=c("pred","real"))
  Tan_sens3_1 = Sensitivity(testdata$STK,pred_Tan3_1)
  Tan_spec3_1 = Specificity(testdata$STK,pred_Tan3_1)
  Tan_precision3_1 = Precision(testdata$STK,pred_Tan3_1)
  Tan_f13_1 = F1_Score(testdata$STK,pred_Tan3_1)
  Tan_roc3_1 = roc(testdata$STK,apply(Tan_prob3_1[,9:16],1,sum),quiet=T)$auc[1] ################### ROC / AUC
  ...
  for(j in 1:length(k_ne)){
    ...
    for(i in 1:k){
      ...
      trainlabel=train[,'STK']
      validlabel=valid[,'STK']
      valid_fit = knn_hamming(traindata=train[,-10],class='STK',testdata=valid[,-10],k=k_ne[j])
  ...
  test_label = testdata[,'STK']
  
  pred <- knn_hamming(traindata , class="STK",testdata, k=opt_k)
  knn_tab = table(pred$new_class,testdata$STK)
  ...
  fit3 = bnc('nb','STK',traindata,smooth=1)
  fit3_pred=predict(fit3,testdata)
  Naive_prob = predict(fit3,testdata,prob=T)
  Naive_tab = table(fit3_pred,testdata$STK)
  
  Naive_sens = Sensitivity(testdata$STK,fit3_pred)
  Naive_spec = Specificity(testdata$STK,fit3_pred)
  Naive_precision = Precision(testdata$STK,fit3_pred)
  Naive_f1 = F1_Score(testdata$STK,fit3_pred)
  
  Naive_acc = accuracy(fit3_pred,testdata$STK)  #####################3 Acc of Naive
  Naive_roc = roc(testdata$STK,Naive_prob[,2],quiet=T)$auc[1] ################# AUC of Naive
  ...
  tree_fit = rpart(STK~.,data=traindata , method='class')
  ...
  tree_acc=accuracy(tree_fit_pred,testdata$STK)
  tree_fit_prob = predict(tree_fit,testdata)
  tree_tab = table(tree_fit_pred,testdata$STK)
  
  tree_sens = Sensitivity(testdata$STK,tree_fit_pred)
  tree_spec = Specificity(testdata$STK,tree_fit_pred)
  tree_precision = Precision(testdata$STK,tree_fit_pred)
  tree_f1 = F1_Score(testdata$STK,tree_fit_pred)
  tree_roc = roc(testdata$STK,tree_fit_prob[,2],quiet=T)$auc[1] ################# AUC of Tree
  ...
  for(j in 1:length(trees)){
    ...
    for(i in 1:k){
      train = traindata[which(traindata$id!=k),]
      valid = traindata[which(traindata$id==k),]
      valid_fit = randomForest(STK~.,data=subset(train, select=-id),mtry=sqrt(length(traindata)-1),ntree=trees[j])
      valid_pred = predict(valid_fit,newdata = subset(valid,select=-id),type='class')
      valid_acc = accuracy(valid_pred,valid[,'STK'])
      valid_result = c(valid_result,valid_acc)
    }
  ...
  traindata = subset(traindata,select=-id)
  ################################# Fitting RF
  RF_fit=randomForest(STK~.,data=traindata , mtry=sqrt(length(data)-1), ntree=opt)
  RF_pred = predict(RF_fit, newdata = testdata , type='class')
  RF_prob = predict(RF_fit, newdata = testdata , type ='prob')
  
  RF_tab = table(RF_pred , testdata$STK)
  RF_acc = accuracy(RF_pred,testdata$STK)
  RF_sens = Sensitivity(testdata$STK,RF_pred)
  RF_spec = Specificity(testdata$STK,RF_pred)
  RF_precision = Precision(testdata$STK,RF_pred)
  RF_f1 = F1_Score(testdata$STK,RF_pred)
  
  RF_roc = roc(testdata$STK,RF_prob[,2],quiet=T)$auc[1]################# AUC of RF
  end_rftime = Sys.time()
  ...
  #write.csv(final_result,paste("eye_data_STK_cv_seed=",seed,".csv"))
  #capture.output(results,file=paste("eye_data_STK_seed=",seed,".txt"))
  n=n+1
}
###################### 
...
colnames(final_cv_result) = c("Acc","AUC","Sens","Spec","Precision","F1","Time")

write.csv(final_cv_result,paste("STK_Total.csv"))
